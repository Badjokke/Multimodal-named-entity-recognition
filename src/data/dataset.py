...
async def load_twitter_dataset(text_processor: Callable[[str], Future[str]], image_processor: Callable[[bytes, str], Future[bytes]]):
    # text_task = asyncio.create_task(__load_twitter_text_dataset(text_processor))
    image_task = asyncio.create_task(__load_twitter_image_dataset(image_processor, filesystem.save_file))
    await image_task
    # result = await asyncio.gather(text_task, image_task)
    return image_task
...
async def __process_image(image_processor: Callable[[bytes, str], Future[bytes]], queue: asyncio.Queue[tuple[str, bytes]],
                          file_writer: Callable[[str, bytes], Coroutine]):
