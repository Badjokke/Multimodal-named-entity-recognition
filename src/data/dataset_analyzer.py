class DatasetAnalyzer:
    def __init__(self, dataset: dict[str, list], class_occurrences: list[int], labels: dict[str, int],
                 vocabulary: dict[str, int]):
        assert "train" in dataset and "test" in dataset and "val" in dataset, "dataset must have keys: train test and val"
        assert len(dataset["train"][0]) == 3 == len(dataset["test"][0]) == len(
            dataset["val"][0]), "each subset must have text, image, label structure"
    ...
    @staticmethod
    def __stats(data: list[tuple[list[str], list[int], list[int]]], label_id_to_label: dict[int, str]) -> dict[
        str, dict | int]:
        ...
        return {"hist": label_histogram, "token_count": token_count, "unique_tokens": len(unique_tokens),
                "sentence_count": sample_size, "image_count": image_count,
                "labels_total": sum(label_histogram.values())}
